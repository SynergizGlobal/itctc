...
const MEASUREMENT_ROWS = 15;

const originGaps = [74, 71, 73, 76, 72, 70, 75, 73, 72, 76, 73, 74, 71, 74, 72];
const endGaps = [76, 73, 75, 78, 74, 72, 77, 75, 74, 78, 75, 76, 73, 76, 74];
const railTemps = [40, 41, 39, 42, 40, 43, 41, 40, 42, 39, 41, 40, 43, 42, 41];
...
export default function FormT122() {
...
<button type="button" onClick={() => navigate(-1)} title="Back" style={{ border: 'none', background: 'transparent', padding: 0, cursor: 'pointer' }}><ArrowLeft aria-hidden="true" /></button>
        <h1 className="h6 mb-0">Form T-12-2</h1>
        <span className="title-main text-center flex-grow-1 mx-3">Measurement record of Expansion Joint (EJ)</span>
        <span>No. <input type="text" defaultValue="T122-001" className="d-inline-block" style={{ width: '60px', border: 'none', borderBottom: '1px solid #000', textAlign: 'center', background: 'transparent', outline: 'none' }} /></span>
        ...
        <div className="form-export-actions">
          ...
          <button type="button" onClick={() => downloadExcel('Form-T-12-2.xls')} title="Download as Excel"><i className="fa-solid fa-file-excel" /></button>
        </div>
      </div>
        ...
        <table className="table table-bordered table-striped table-sm align-middle form-table export-table mb-0" border="1">
          <thead>
            <tr>
              ...
              <th width="90" rowSpan="2">EJ No.</th>
              <th colSpan="2">Chainage</th>
              <th width="120" rowSpan="2">EJ Gap - Origin<br />(mm)</th>
              <th width="120" rowSpan="2">EJ Gap - End<br />(mm)</th>

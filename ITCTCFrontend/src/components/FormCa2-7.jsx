...
function measurementRow(index) {
  const base = 700 + index * 30;
  return {
    sl: String(index + 1).padStart(2, '0'),
    point: `T${index + 1}`,
    km: Math.floor(base / 1000),
    m: base % 1000,
    visual: index % 2 === 0 ? 'OK' : 'Crack',
    hammer: index % 3 === 0 ? 'Normal' : 'Hollow',
    remarks: '',
  };
}
              ...
              {Array.from({ length: MEASUREMENT_ROWS }, (_, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td className="text-center">{r.visual}</td>
                    <td className="text-center">{r.hammer}</td>
                    <td>{r.remarks || '\u00a0'}</td>
                  </tr>
                );
              })}
